"""Vendored OralB toothbrush BLE client.

Vendored from https://github.com/bkbilly/oralb (v0.2, commit e1111ef) because the
published `oralb==0.2` PyPI package has invalid metadata that HA's uv installer
rejects (RequirementsNotFound). Deps (bleak, bleak-retry-connector) already ship
in HA core, so no manifest requirement is needed.
"""
